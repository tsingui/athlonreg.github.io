...
```bash
# vi /usr/local/tomcat/webapps/cas/WEB-INF/classes/serviies/OAUTH-1002.json
```
...
注释掉默认的 `cas.authn.accept.users` 认证方式并添加与 LDAP Server 连接的配置
...
cas.authn.ldap[0].dnFormat=cn=%s,ou=People,dc=iamzhl,dc=top
